...
Use Poisson regression model since data are counts (positive integers only)
```{r}
...
# No issue; interpret output
summary(mod1)

```

No sig. interactions between site, season, invert diversity; simplify to run as additive model
```{r}
...
We did not see significant differences in fish taxon richness between fall and spring seasons
...
ggplot() +
  geom_boxplot(data = fish_by_season, aes(x = season, y = fish_unique_taxa)) +
  theme_bw()
```
